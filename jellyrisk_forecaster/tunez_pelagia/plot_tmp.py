...
def delete_for_date1(target_date, table=settings.CARTODB_TABLE1):
...
def construct_query1(target_date, limit_rows=settings.LIMIT_ROWS):
    values = []
    ...
    query = 'INSERT INTO %s (lon, lat, prob, probinf, probsup, date, the_geom) VALUES %s;'  \
            % (settings.CARTODB_TABLE1, ', '.join(values[:limit_rows]))
    return query
...
def plot1(target_date, delete_existing=True):
    ...
    if delete_existing:
        delete_for_date1(target_date)

    query = construct_query1(target_date)
    insert_data1(query)
