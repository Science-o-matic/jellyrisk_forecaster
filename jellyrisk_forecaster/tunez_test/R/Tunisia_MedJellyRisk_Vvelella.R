...
#----------------------------------------------------#
#Cargamos historial
load("Tunisia_MedJellyRisk_Vvelella.RData") # 
...
#Guardamos historial
save.image("Tunisia_MedJellyRisk_Vvelella.RData")
...
#x11()
#level.plot(data.in=myResp, XY=myRespXY, color.gradient = "red", cex = 1, level.range = c(min(myResp), max(myResp)), show.scale = TRUE, title = "Velella velella", SRC=FALSE, save.file="no", ImageSize="large", AddPresAbs=NULL)
...
# GAM average
proj.gam <- rowMeans(cbind(get_predictions(myBiomodProj, as.data.frame=TRUE)$Vv_AllData_RUN1_GAM, get_predictions(myBiomodProj, as.data.frame=TRUE)$Vv_AllData_RUN2_GAM, get_predictions(myBiomodProj, as.data.frame=TRUE)$Vv_AllData_RUN3_GAM))/1000

#x11()
#level.plot(data.in=proj.gam, XY=myRespXY, color.gradient = "red", cex = 1, level.range = c(min(proj.gam), max(proj.gam)), show.scale = TRUE, title = "Velella velella Projection \n (GAM)", SRC=FALSE, save.file="no", ImageSize="large", AddPresAbs=NULL)
...
#x11()
#qplot(x=Long, y=Lat, colour=Ensemble_Mean, data=Ens.Forecast, xlab="longitude", ylab="Latitude", main="V. velella ensemble forecast") +
...
write.csv(data_to_write,
	    file=sprintf("./Projections/VvelellaEF-%s.csv", target_date),
	      row.names=FALSE
	    )
...
ensforecast.sat <- 
  ...
  labs(title ="V. velella ensemble forecast", x="Longitude", y="Latitude", colour="Mean\nProbability")+
  theme_bw()

#x11()
#ensforecast.sat

#ggsave(filename="V.velella.Ensforcast.sat.jpeg", plot=ensforecast.sat, path=getwd(), width=20, height=20, units="cm", dpi = 300)
