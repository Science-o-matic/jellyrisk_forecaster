...
#----------------------------------------------------#
#Cargamos historial
load("Tunisia_MedJellyRisk_Aaurita.RData") # 
...
#Guardamos historial
save.image("Tunisia_MedJellyRisk_Aaurita.RData")
...
#x11()
#level.plot(data.in=myResp, XY=myRespXY, color.gradient = "red", cex = 1, level.range = c(min(myResp), max(myResp)), show.scale = TRUE, title = "Aurelia aurita", SRC=FALSE, save.file="no", ImageSize="large", AddPresAbs=NULL)
...
# GAM average
proj.gam <- rowMeans(cbind(get_predictions(myBiomodProj, as.data.frame=TRUE)$Aa_AllData_RUN1_GAM, get_predictions(myBiomodProj, as.data.frame=TRUE)$Aa_AllData_RUN2_GAM, get_predictions(myBiomodProj, as.data.frame=TRUE)$Aa_AllData_RUN3_GAM))/1000

#x11()
#level.plot(data.in=proj.gam, XY=myRespXY, color.gradient = "red", cex = 1, level.range = c(min(proj.gam), max(proj.gam)), show.scale = TRUE, title = "Aurelia aurita Projection \n (GAM)", SRC=FALSE, save.file="no", ImageSize="large", AddPresAbs=NULL)
...
names(Ens.Forecast)
colnames(Ens.Forecast) <- c('Pn_Ensemble_Mean','Pn_Ensemble_CV','Pn_Ensemble_ciInf','Pn_Ensemble_ciSup','Pn_Ensemble_Median','Pn_Ensemble_ca','Pn_Ensemble_WMean')
...
#x11()
#qplot(x=Long, y=Lat, colour=Pn_Ensemble_Mean, data=Ens.Forecast, xlab="longitude", ylab="Latitude", main="A. aurita ensemble forecast") +
...
data_to_write <- data.frame(
	...
	prob=Ens.Forecast$Pn_Ensemble_Mean,
	probsup=Ens.Forecast$Pn_Ensemble_ciSup,
	probinf=Ens.Forecast$Pn_Ensemble_ciInf,
	date=rep(paste(target_date, '00:00:00'), length(Ens.Forecast$Long))
			     )

write.csv(data_to_write,
	    file=sprintf("./Projections/AauritaEF-%s.csv", target_date),
	      row.names=FALSE
	    )
...
#x11()
#ggmap(Tunisia.cEns.Forecast)

ensforecast.sat <- 
  ggmap(Tunisia.coast, legend='none') +
  geom_point(aes(x=Long, y=Lat, colour=Pn_Ensemble_Mean, size=1.5), data= Ens.Forecast, alpha= 0.9, labels="")+
  scale_colour_gradient(low="gray", high="red",space ="Lab", na.value = "grey50", guide = "colourbar")+
  scale_size_continuous(1.5, guide="none")+
  labs(title ="A. aurita ensemble forecast", x="Longitude", y="Latitude", colour="Mean\nProbability")+
  theme_bw()

#x11()
#ensforecast.sat

#ggsave(filename="A.aurita.Ensforcast.sat.jpeg", plot=ensforecast.sat, path=getwd(), width=20, height=20, units="cm", dpi = 300)
